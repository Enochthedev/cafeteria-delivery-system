...
export class BasicConfig {
  @IsString()
  @IsIn(['/api/v1', '/api/v2'], { message: 'Invalid API version' })
  api_version: string = '/api/v1';

  @IsString()
  @IsIn(['dev', 'test', 'production', 'staging'], { message: 'Invalid node environment' })
  node_env: string = 'dev';
...
export function loadEnv<T extends BasicConfig>(ConfigClass: new (config?: Partial<T>) => T): T {
  ...
  try {
    return plainToClass(ConfigClass, processedEnv);
  } catch (err) {
...
export const basicEnvConfig = loadEnv(BasicConfig);